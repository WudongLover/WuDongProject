import { Body, Controller, Del, Get, Inject, Param, Patch, Post, Query } from '@midwayjs/core'; import { InjectEntityModel } from '@midwayjs/typeorm'; import { Repository } from 'typeorm'; import { HomestayService } from '../service/homestay_service'; import { HomestayEntity } from '../entity/homestay_entity'; import { RoomTypeEntity } from '../entity/room_type_entity'; import { RoomCalendarEntity } from '../entity/room_calendar_entity';
@Controller('/api/app/m3/homestay') export class HomestayController { @Inject() service: HomestayService; @InjectEntityModel(HomestayEntity) homestays:Repository<HomestayEntity>; @InjectEntityModel(RoomTypeEntity) rooms:Repository<RoomTypeEntity>; @InjectEntityModel(RoomCalendarEntity) calendars:Repository<RoomCalendarEntity>;
  ...
  @Get('/room-calendar/:roomTypeId') async calendar(@Param('roomTypeId') id:string){ return this.ok(await this.calendars.find({where:{roomTypeId:id},order:{date:'ASC'}})); }
