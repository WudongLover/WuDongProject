...
@Controller('/api/user') export class AuthController { @Inject() service:AuthService; @Inject() ctx:Context;
 ...
 @Get('/me') async me(){return{code:0,message:'ok',data:await this.service.me((this.ctx as any).userId)};} @Post('/profile') async profile(@Body()b:any){return{code:0,message:'ok',data:await this.service.me((this.ctx as any).userId)};}
}
