/**
 * 阿里云 OSS 工具类（乌东文旅 · 后台后端）
 *
 * 定位：项目中所有照片（商品/民宿/门票/推文封面、头像等）的对象存储底座，
 * 并作为 cool-admin 文件上传插件（modules/plugin/hooks/upload）的 OSS 后端。
 * 约定：
 ...
 *
 * 与 wu_dong_midway/src/comm/oss.ts 保持同一套 API 与环境变量名（两个后端无法共享代码，各自维护一份）。
 */
