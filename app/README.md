...
- `app/cool-admin-midway`（端口 **8001**）是管理后台服务，承载 Cool Admin 的后台登录、RBAC、商家/平台管理接口；后台接口使用 `/admin/*` 或框架约定的管理端路径。
- 两套服务可以共用 `wudong` 数据库，但不得互相复用控制器、路由或鉴权中间件；跨服务联动通过稳定的 HTTP API 或公共数据契约完成。
- C 端前端 `wu_dong_vue` 仅代理到 6666；管理后台前端 `cool_admin_vue` 仅代理到 8001，禁止在前端模块中混用另一套服务的内部接口。
- m3/m4 的公开查询接口允许游客访问，新增、更新、删除等写操作必须经过 C 端鉴权；管理端 CRUD 则由 8001 的后台权限体系保护。
